-- ══════════════════════════════════════════════════════════
--  Auto state transitions per tipo durata airdrop
--  presale → sale (quando % blocchi venduti raggiunge soglia)
--  sale → closed + auto-draw (quando 100% venduto)
-- ══════════════════════════════════════════════════════════

-- ── Config keys per soglie presale→sale ──
INSERT INTO airdrop_config (key, value, description) VALUES
  ('presale_threshold_flash', '30', 'Soglia % blocchi venduti per presale→sale (Flash 6h)'),
  ('presale_threshold_standard', '20', 'Soglia % blocchi venduti per presale→sale (Standard 24h)'),
  ('presale_threshold_extended', '15', 'Soglia % blocchi venduti per presale→sale (Extended 72h)'),
  ('auto_close_on_sellout', 'true', 'Chiudi e estrai automaticamente quando 100% venduto')
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, description = EXCLUDED.description;

-- ── buy_blocks v3: con auto-transizione stato ──
...
BEGIN
  ...
  IF v_new_sold >= v_airdrop.total_blocks THEN
    ...
    IF v_auto_close = 'true' AND v_airdrop.auto_draw THEN
      ...
      v_new_status := 'closed';
      -- Il draw verrà eseguito dal cron auto_draw esistente o manualmente
    END IF;
  END IF;
