-- Fix: handle auto-created profile from auth trigger
-- Supabase Auth trigger creates a profile row on auth.users INSERT,
-- so we need ON CONFLICT to upsert with test data.
...
BEGIN
  ...
  FOR i IN (v_existing + 1)..1000 LOOP
    ...
    -- Upsert: auth trigger may have already created the profile row
