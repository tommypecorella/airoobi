-- ══════════════════════════════════════════════════════════
--  Aggiunge stato 'valutazione_completata' al ciclo di vita airdrop
--  + aggiorna whitelist manager_update_airdrop con 'valutazione_completata' e 'accettato'
--
--  Workflow aggiornato (14 Apr 2026):
--  DRAFT → IN_VALUTAZIONE → VALUTAZIONE_COMPLETATA → ACCETTATO → PRESALE/SALE → CLOSED → COMPLETED
--                         ↘ rifiutato_min500 / rifiutato_generico
--  (qualsiasi momento)    → ANNULLATO (utente ritira, perde ARIA)
--
--  VALUTAZIONE_COMPLETATA = staff ha completato la valutazione, utente deve decidere
--  ACCETTATO = utente ha accettato la valutazione, airdrop parte in presale/sale
-- ══════════════════════════════════════════════════════════

-- Aggiorna constraint status con nuovo stato
...
-- Aggiorna manager_update_airdrop: aggiunge 'valutazione_completata' e 'accettato' alla whitelist
...
BEGIN
  ...
  -- Whitelist stati settabili da staff (ABO)
  IF p_status NOT IN (
    'in_valutazione',
    'valutazione_completata',
    'accettato',
